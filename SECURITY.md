...
We appreciate your participation in keeping the IOTA network secure. This doc includes details on IOTA's Bug Bounty Program, including information on how to report an issue, what bug bounties are available, and who is eligible for bounties.

The program is run in partnership with [HackenProof](https://hackenproof.com/). More information is available on [their site](https://hackenproof.com/programs/iota-protocol).
...
All reports must be sent using HackenProof’s [secure dashboard](https://hackenproof.com/programs/iota-protocol). **DO NOT report a security issue using GitHub, email, or Discord.**
...
If you are reporting something outside the scope of HackenProof's Bug Bounty Program for Iota, contact [security@iota.io](mailto:security@iota.io)

# Bug bounties

The IOTA Foundation offers bug bounties for security issues found at different levels of severity. If a vulnerability is found, follow the process detailed above to report the issue. All bug reports must come with a runnable [testnet](https://github.com/iotaledger/iota/tree/testnet) Proof-of-Concept with an end-effect impacting an asset-in-scope in order to be considered for a reward. You do not need to have a fix in order to submit a report or receive a bounty. By your participation in IOTA Foundation’s bug bounty program you agree to be bound and abide by our [terms of service](https://iota.org/terms) and [privacy policy](https://iota.org/policy) .

## Assets in scope

Impacts only apply to assets in active use by IOTA.
...
The following impacts are accepted within this bug bounty program--refer to [IOTA's HackenProof Bug Bounty Program Page](https://hackenproof.com/iota/iota/) for an official and up-to-date listing.
All other impacts are considered out-of-scope and ineligible for payout.

CRITICAL [$100,000-$500,000 USD]

1. Exceeding the maximum supply of 10 billion IOTA + allowing the attacker to claim the excess funds
2. Loss of Funds which includes
   - Unauthorized creation, copying, transfer or destruction of objects via bypass of or exploit of bugs in the Move or IOTA bytecode verifier
   - Address Collision – creating two distinct authentication schemes that hash to the same IOTA address in a manner that lead to significant loss of funds
...
2. Unintended, permanent burning of IOTA under the max cap.
...
2. A remote call that crashes a IOTA fullnode
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| In our staking contract, we have the concept of pool tokens and keep track of exchange rates between pool tokens and IOTA tokens of all epochs, which increase as more rewards are added to the staking pools. When a user withdraws their stake, we retrieve from that record both the exchange rate at staking time and the current exchange rate (at withdrawing time), and calculate the rewards to be paid out based on the difference in exchange rates. While doing this calculation, we do conversions both ways: pool tokens -> IOTA and IOTA -> pool tokens. Rounding may happen along the way due to integer division. The exchange rate between the two tokens should stay roughly the same since we will be burning a proportional amount of pool tokens as IOTA is withdrawn. However, in the extreme case where a user is unstaking 1 NANOS, this rounding error may cause ZERO pool tokens to be burnt, causing the pool token to effectively depreciate. If an attacker has a lot of 1 NANOS stakes, they can withdraw them one by one, causing the pool token exchange rate to drop and other takers to “lose” their staking rewards. I put quotation marks around “lose” because the attacker themselves won’t get any of that rewards so this attacker doesn’t actually make economic sense. Rather the rewards stay in the rewards pool and will become dust. This issue is mitigated by enforcing a minimum staking amount of 1 IOTA or 10^9 NANOS in this PR: https://github.com/iotaledger/iota/pull/9961 | Critical - Any other issue leading to theft or loss of valuable objects, with severity depending on the consequences of the issue and the preconditions for exploiting it |
...
- Bounties are awarded on a rolling basis and eligibility to receive such awards shall be determined by IOTA Foundation in its sole discretion.
...
- Participants must use the [HackenProof dashboard](https://hackenproof.com/iota/iota) to report bugs or vulnerabilities. Reports made via email, Discord, or Twitter will not be eligible for bounties.
...
- Attempting phishing or other social engineering attacks against IOTA or HackenProof employees and/or customers
...
_All of the program details along with a link to the dashboard to report a bug are available on [HackenProof’s bounty program page for IOTA](https://hackenproof.com/iota/iota/)._

**How do I join the program?**

_If you find a bug or vulnerability, report it using the [HackenProof dashboard](https://hackenproof.com/iota/iota). You should receive an acknowledgement of your report within 48 hours for critical vulnerabilities and 96 hours for all other vulnerabilities._

**Where can I get technical questions answered?**

_Iota and HackenProof will be conducting Office Hours to answer questions. A date will be announced on Twitter by [@IotaNetwork](https://x.com/suinetwork). If you are not able to attend, you can email questions to info@hackenproof.com._ with questions regarding the Bug Bounty Program.

For additional security concerns/questions/comments outside the scope of the HackenProof Bug Bounty Program, reach out to IOTA's Community [Discord](https://discord.gg/iota), [Forums](https://forums.iota.io/), or e-mail us at _security@iota.io_

**Who is behind this program?**

_The program is funded and managed by the IOTA Foundation, in partnership with HackenProof._
