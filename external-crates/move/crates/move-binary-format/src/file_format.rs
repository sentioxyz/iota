...
//!   that starts every Diem binary, followed by the version of the VM used to
//!   compile the code, and last is the number of tables present in this binary.
//! - **Table Specification**: it's a number of tuple of the form `(table type,
//!   starting_offset, byte_count)`. The number of entries is specified in the
//!   header (last entry in header). There can only be a single entry per table
//!   type. The `starting offset` is from the beginning of the binary. Tables
//!   must cover the entire size of the binary blob and cannot overlap.
//! - **Table Content**: the serialized form of the specific entries in the
//!   table. Those roughly map to the structs defined in this module. Entries in
...
/// A `DatatypeHandle` is a reference to a user defined type. It is composed by
/// a `ModuleHandle` and the name of the type within that module.
///
/// A type in a module is uniquely identified by its name and as such the name
/// is enough to perform resolution.
///
/// The `DatatypeHandle` is polymorphic: it can have type parameters in its
/// fields and carries the ability constraints for these type parameters (empty
/// list for non-generic structs). It also carries the abilities of the struct
/// itself so that the verifier can check ability semantics without having to
...
pub struct StructDefinition {
    /// The `DatatypeHandle` for this `StructDefinition`. This has the name and
...
pub struct EnumDefinition {
    /// The `DatatypeHandle` for this `EnumDefinition`. This has the name and
    /// the abilities for the type.
...
pub struct VariantHandle {
    ...
    /// The tag of this variant. Note that this is the index of the variant in
    /// the `EnumDefinition` `variants` field.
...
pub struct VariantInstantiationHandle {
    /// The `EnumDefInstantiationIndex` that this `VariantInstantiationHandle`
    /// is part of.
...
pub enum Bytecode {
    ...
    /// (according to its type) from the `ConstantPool` via
    /// `ConstantPoolIndex`
    ///
    /// Stack transition:
    ///
    ...
    /// Create an instance of the type specified via `DatatypeHandleIndex` and
    ...
    CastU256,
    /// Create a variant of the enum type specified via `VariantHandleIndex` and
    /// push it on the stack. The values of the fields of the variant, in
    /// the order they appear in the variant declaration, must be pushed on
    /// the stack. All fields for the variant must be provided.
    ///
    /// A PackVariant/PackVariantGeneric instruction must fully initialize an
    /// instance.
    ///
    /// Stack transition:
    ///
    ...
    /// Destroy a variant value specified by the `VariantHandleIndex` and push
    /// the values bound to each variant field on the stack.
    ///
    /// The values of the fields of the instance appear on the stack in the
    /// order defined in the enum variant's definition.
    ///
    /// This order makes UnpackVariant<T>(tag) the inverse of
    /// PackVariant<T>(tag). So `UnpackVariant<T>(tag); PackVariant<T>(tag)`
    /// is the identity for enum T and variant V with tag `t`.
    ///
    /// Stack transition:
    ///
    /// ```..., instance_value -> ..., field(1)_value, field(2)_value, ...,
    ...
    /// Branch on the tag value of the enum value reference that is on the top
    /// of the value stack, and jumps to the matching code offset for that
    /// tag within the `CodeUnit`. Code offsets are relative to the start of
    /// the instruction stream.
    ///
    /// Stack transition:
    /// ```..., enum_value_ref -> ...```
...
impl Bytecode {
    ...
    pub fn get_successors(
        ...
        // NB: the size of `v` is generally quite small (bounded by maximum # of
        // variants allowed in a variant jump table), so a sort here is not a
        // performance concern.
        v.sort();
