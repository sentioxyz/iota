...
module nft_marketplace::rental_extension {
    ...
    public struct RentalPolicy<phantom T> has key, store {
        ...
        ///
        /// For example, fpercentages can be represented using basis points:
    ...
    public fun remove(kiosk: &mut Kiosk, cap: &KioskOwnerCap, _ctx: &mut TxContext) {
        kiosk_extension::remove<Rental>(kiosk, cap);
    }
    ...
    /// Marketplace extension installed.
    ///
    ...
    public fun rent<T: key + store>(
        ...
        assert!(kiosk_extension::is_installed<Rental>(borrower_kiosk), EExtensionNotInstalled);
    ...
    public fun reclaim<T: key + store>(
        ...
        assert!(kiosk_extension::is_installed<Rental>(renter_kiosk), EExtensionNotInstalled);
        ...
        if (transfer_policy.has_rule<T, LockRule>()) {
            ...
            kiosk_extension::place<Rental, T>(
                Rental {},
                renter_kiosk,
                object,
                transfer_policy,
