...
const protocolInject = async function (source) {
  ...
  const iotaSorted = (array) => {
    return array.sort((a, b) => {
      const aStartsWithIota = a.name.startsWith("iota");
      const bStartsWithIota = b.name.startsWith("iota");

      if (aStartsWithIota && !bStartsWithIota) return -1;
      if (!aStartsWithIota && bStartsWithIota) return 1;
      return 0;
    });
  };
  ...
  const files = iotaSorted(spec.files);
  const bordercolor = "border-iota-blue-dark dark:border-iota-blue";

  const tabStyle = `grid grid-cols-6 border border-solid border-b-0 border-iota-blue-dark ${bordercolor}`;
  const tabRowStyle = `p-4 border-0 border-b border-solid ${bordercolor} col-span-full`;
  const tabHeaderStyle = `${tabRowStyle} bg-iota-gray-50 dark:bg-iota-ghost-dark`;
  const tabAltHeaderStyle = `${tabRowStyle} bg-iota-ghost-white dark:bg-iota-gray-95`;
      ...
      if (fields.length > 0) {
        for (const field of fields.entries()) {
          ...
          if (hasLabel) {
            ...
            let labelBg = "bg-iota-ghost-white dark:bg-iota-ghost-dark";
            if (field[1].label === "optional") {
              label = "Proto3 optional";
              labelBg = "bg-iota-blue-light dark:bg-iota-blue-dark";
            } else if (field[1].label === "repeated") {
              label = "Repeated []";
              labelBg = "bg-iota-warning-light dark:bg-iota-warning-dark";
            }
            content.push(
  ...
  const titleStyle =
    "p-4 pb-2 font-bold text-iota-ghost-dark dark:text-iota-ghost-white bg-iota-gray-50 dark:bg-iota-ghost-dark border border-solid border-transparent rounded-t-lg";
  const valStyle =
