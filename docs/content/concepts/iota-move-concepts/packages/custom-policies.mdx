...
IOTA comes with a set of built-in package compatibility policies, listed here from most to least strict:
...
You can change the current policy by calling one of the functions in `iota::package` (`only_additive_upgrades`, `only_dep_upgrades`, `make_immutable`) on the package's `UpgradeCap` and a policy can become only more restrictive. For example, after you call `iota::package::only_dep_upgrades` to restrict the policy to become additive, calling `iota::package::only_additive_upgrades` on the `UpgradeCap` of the same package results in an error.
...
While step 2 is a built-in command, steps 1 and 3 are implemented as Move functions. The IOTA framework provides their most basic implementation:

{@inject: crates/iota-framework/packages/iota-framework/sources/package.move#fun=authorize_upgrade,commit_upgrade noComments}

These are the functions that `iota client upgrade` calls for authorization and commit. Custom upgrade policies work by guarding
...
{@inject: crates/iota-framework/packages/iota-framework/sources/package.move#struct=UpgradeCap noComments}
...
{@inject: crates/iota-framework/packages/iota-framework/sources/package.move#struct=UpgradeTicket noComments}
...
bytecode or source code for ahead of time. IOTA calculates the digest as follows:
...
Refer to the [implementation for digest calculation](https://github.com/iotaledger/iota/blob/d8cb153d886d54752763fbdab631b062da7d894b/crates/iota-types/src/move_package.rs#L232-L251) for more information, but in most cases, you can rely on the Move toolchain to output the digest as part of the build, when passing the `--dump-bytecode-as-base64` flag:
...
{@inject: crates/iota-framework/packages/iota-framework/sources/package.move#struct=UpgradeReceipt noComments}

The `UpgradeReceipt` is proof that the `Upgrade` command ran successfully, and IOTA added the new package to the set of created
...
After IOTA creates an `UpgradeReceipt`, you must use it to update its `UpgradeCap` within the same transaction (you cannot store it for later, drop it, or burn it), or the transaction fails.
...
Finally, provide implementations of `commit_upgrade` and `make_immutable` that delegate to their respective functions in `iota::package`:
...
Use the `iota client publish` command to publish the policy.
...
If you don't have a package available, use the `iota move new` command to create the template for a new package called `example`.
...
Open a terminal or console to the root of your Node.js project. Run the following command to add the IOTA TypeScript SDK as a dependency:
...
- `IOTA`: the location of the `iota` CLI binary.
...
Next, add boilerplate code to get the signer key pair for the currently active address in the IOTA Client CLI:
...
Use the IOTA Client CLI to test the upgraded package (the package ID is **different** from the original version of your example package):
