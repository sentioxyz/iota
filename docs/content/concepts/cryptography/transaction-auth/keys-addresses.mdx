...
The `coin_type` value is managed with a repository of all other cryptocurrencies. Both signature schemes use IOTA's registered coin_type, 784 (IOTA on a telephone keypad).
...
```sh
$ iota keytool import "TEST_MNEMONIC" ed25519 "m/44'/784'/0'/0'/0'"
```
```sh
$ iota client new-address ed25519 "m/44'/784'/0'/0'/0'"
```
