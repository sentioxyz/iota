...
zkLogin is a IOTA primitive that enables you to send transactions from a IOTA address using an OAuth credential, without publicly linking the two.

Go to [zkLogin](./cryptography/zklogin.mdx).

## Passkey

IOTA supports the passkey signature scheme that enables you to sign-in to apps and sign transactions for IOTA using a private key securely stored on a passkey authenticator. It uses the WebAuthn standard.
