...
description: Dynamic fields and dynamic object fields on IOTA are added and removed dynamically, affect gas only when accessed, and store heterogeneous values.
draft: true
---
...
Fortunately, IOTA provides *dynamic fields* with arbitrary names (not just identifiers), added and removed on-the-fly (not fixed at publish), which only affect gas when they are accessed, and can store heterogeneous values. Use the libraries in this topic to interact with this kind of field.
...
| Object field | Values must be objects (have the `key` ability, and `id: UID` as the first field), but are still accessible at their ID to external tools.| [`dynamic_object_field`](https://github.com/iotaledger/iota/tree/main/crates/iota-framework/packages/iota-framework/sources/dynamic_object_field.move) |
...
Use the `add` function from the relevant IOTA framework module to add dynamic fields:

**Dynamic field**

{@inject: crates/iota-framework/packages/iota-framework/sources/dynamic_field.move#fun=add noComments}

**Dynamic object field**

{@inject: crates/iota-framework/packages/iota-framework/sources/dynamic_object_field.move#fun=add noComments}
...
{@inject: crates/iota-framework/packages/iota-framework/sources/dynamic_field.move#fun=borrow,borrow_mut noComments}

Where `object` is the UID of the object the field is defined on and `name` is the field's name.

:::info

`iota::dynamic_object_field` has equivalent functions for object fields, but with the added constraint `Value: key + store`.
...
{@inject: crates/iota-framework/packages/iota-framework/sources/dynamic_field.move#fun=remove noComments}
...
`iota::dynamic_object_field` has an equivalent function for object fields.
...
IOTA provides `Table` and `Bag` collections built using dynamic fields, but with additional support to count the number of entries they contain to protect against accidental deletion when non-empty. To learn more, see [Tables and Bags](./dynamic-fields/tables-bags.mdx).
