...
```sh
$ curl -i -X POST https://iota-mainnet.iota.org/graphql \
...
```sh
$ curl -X POST https://iota-testnet.iota.org/graphql \
    --header 'Content-Type: application/json' \
    --data '{
...
- `pageInfo`, of type <a href="/references/iota-api/iota-graphql/reference/objects/page-info" data-noBrokenLinkCheck='true'>PageInfo</a>, which indicates whether there are more pages before or after the page returned.
...
- [Querying IOTA RPC with GraphQL](../guides/developer/getting-started/graphql-rpc.mdx): Gets you started using GraphQL to query the IOTA RPC for on-chain data.  
- [Migrating to GraphQL](../guides/developer/advanced/graphql-migration.mdx): Guides you through migrating IOTA RPC projects from JSON-RPC to GraphQL. 
- [GraphQL for IOTA RPC](../references/iota-graphql.mdx): Auto-generated GraphQL reference for IOTA RPC.
