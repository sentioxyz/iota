...
IOTA uses the Mysticeti protocol to optimize for both low latency and high throughput. The benefits of Mysticeti include:
...
- [Life of a Transaction](./transaction-lifecycle.mdx): The life of a transaction on the IOTA network has some differences compared to those from other blockchains.
- [IOTA Security](./iota-security.mdx): IOTA is designed to provide very high security guarantees to asset owners.
