...
This document defines the styles, vocabulary usage, and content formatting for IOTA documentation. Entries are in alphabetical order. A style guide is never finished. Expect continued iterations to add additional styles, additional information to existing styles, and infrequently a change to an existing style.
...
> To learn more, see [Examples of IOTA Smart Contracts](/guides/developer/app-examples.mdx).
...
> Before you install Iota, make sure to install the [prerequisites](/guides/developer/getting-started/iota-install.mdx#prerequisites).
