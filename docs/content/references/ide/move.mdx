...
1. Search for `iota.move`.
1. Click **Install** on the Move extension by IOTA Foundation.

Alternative install methods include:

  - Use <kbd>Ctrl</kbd> + <kbd>P</kbd> or <kbd>⌘</kbd> + <kbd>P</kbd> and type `ext install iota.move`.
  - Use the command line: 

      ```sh
      $ code --install-extension iota.move
      ```
...
- [Move Trace Debugger](https://marketplace.visualstudio.com/items?itemName=iota.move-trace-debug)
...
```sh
$ cargo install --git https://github.com/iotaledger/iota.git iota-move-lsp
```

By default, the Move extension expects to find the `move-analyzer` binary in `~/.iota/bin`. You can either copy the binary to this location, or configure the extension to use a different path. 
...
These commands find the `Move.toml` file for the open Move source file and open a terminal to run the appropriate `iota move` command.

To generate a trace, you must have a trace-capable `iota` binary. See [Debugger](./debugger.mdx) for more information.
