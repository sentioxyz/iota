...
- The blockchain-oriented [Move programming language](https://github.com/iotaledger/awesome-move) eases the developer experience
...
## IOTA's approach to validating new transactions {#validating}
...
This doesn't mean that IOTA as a platform never orders transactions with respect to each other, or that it allows owners to affect only their owned microcosm of objects. IOTA also processes transactions that have an effect on some shared state in a rigorous, consensus-ordered manner. See the [State-of-the-art consensus](#state-of-the-art-consensus) section for details on the consensus engine.
...
1.  Sender collects a Byzantine-resistant-majority of these votes into a _certificate_ and broadcasts it to all IOTA validators.
...
## Where IOTA excels {#where-iota-excels}
