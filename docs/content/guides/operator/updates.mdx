...
description: Update your IOTA Full node to the latest version to remain in sync with the network.
---

## IOTA release process

Each IOTA network is deployed on a consistent schedule. There are extenuating circumstances that might delay releases occasionally, but these delays are rare and communicated through [official channels](#communication).
...
For additional details, see each network's [release schedule and configuration](https://iota.org/networkinfo).
...
Releases are announced on [IOTA Discord server](https://discord.com/invite/iota) and [node-operators](https://groups.google.com/a/groups.iota.io/g/node-operators) Google group.
...
You can track the latest version of IOTA on the [IOTA Releases](https://github.com/iotaledger/iota/releases) page on GitHub.
The schedule for each network is available in the [Network Release Schedule](https://iota.org/networkinfo) page.

It is reasonable to have to shut down your Full node to perform an update, whether that be a rolling restart in Kubernetes, or a systemctl stop on a Linux machine to replace the iota-node binary.
