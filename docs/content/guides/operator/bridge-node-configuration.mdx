---
title: IOTA Bridge Validator Node Configuration
sidebar_label: IOTA Bridge Node Configuration
...
To set up and run a Bridge Node, you need to install `iota` and `iota-bridge-cli`. You can install them using one of the following options: 

Install from tip of `main`:
```sh
$ cargo install --locked --git "https://github.com/iotaledger/iota.git" iota iota-bridge-cli
```
Install with a commit sha:
```sh
$ cargo install --locked --git "https://github.com/iotaledger/iota.git" --rev {SHA} iota iota-bridge-cli
```
...
- A REST API URL where the Bridge Node listens to and serves requests. Example: `https://bridge.example-iota-validator.io:443`. Make sure the port is correct and the URL does not contain any invalid characters, like quotes for example.

To create a `BridgeAuthorityKey`, run
```sh
$ iota-bridge-cli create-bridge-validator-key <PATH-TO-WRITE>
```
...
After you have both authority key file and REST API URL ready, you can register them by using IOTA CLI:

```sh
$ iota validator register-bridge-committee --bridge-authority-key-path <BRIDGE-AUTHORITY-KEY-PATH> --bridge-authority-url <BRIDGE-AUTHORITY-URL>
```
...
Both key and URL are changeable **before the committee is finalized**. If you wish to update metadata, simply rerun `iota validator register-bridge-committee`.
...
```sh
$ iota-bridge-cli view-bridge-registration --iota-rpc-url {IOTA-FULLNODE-URL}
```
...
Use `iota-bridge-cli` command to create a template. If you want to run `BridgeClient` (see the following section), pass `--run-client` as a parameter.

```sh
$ iota-bridge-cli create-bridge-node-config-template {PATH}
$ iota-bridge-cli create-bridge-node-config-template --run-client {PATH}
```
...
| `bridge-authority-key-path` | The path to the Bridge Validator key, generated from `iota-bridge-cli create-bridge-validator-key` command referenced previously. |
...
| `iota:iota-rpc-url` | IOTA RPC URL. |
| `iota:iota-bridge-chain-id` | `0` for IOTA Mainnet, `1` for IOTA Testnet. |
...
| `metrics:push-url` | The url of the remote IOTA metrics pipeline (for example, `https://metrics-proxy.[testnet_OR_mainnet].iota.io:8443/publish/metrics`). See the [metrics push section](#metrics-push) that follows for more details. | 
...
| `iota:bridge-client-key-path` | The file path of Bridge Client key. This key can be generated with `iota-bridge-cli create-bridge-client-key` as previously shown. When `run-client` is true but you do not provide `iota:bridge-client-key-path`, it defaults to use the Bridge Validator key to submit transactions on IOTA. This is not recommended for the sake of key separation. | 

### Bridge Client

`BridgeClient` orchestrates bridge transfer requests. It is **optional** to run for a `BridgeNode`. `BridgeClient` submits transaction on the IOTA network. Thus when it's enabled, you need a IOTA account key with enough IOTA balance.
...
```sh
$ iota-bridge-cli create-bridge-client-key <PATH_TO_BRIDGE_CLIENT_KEY>
```
This prints the newly created IOTA Address. Then we need to fund this address with some IOTA for operations.
...
- Use `cargo install`.
    ```sh
    $ cargo install --locked --git "https://github.com/iotaledger/iota.git" --branch {BRANCH-NAME} iota-bridge
    ```
    Or
    ```sh
    $ cargo install --locked --git "https://github.com/iotaledger/iota.git" --rev {SHA-NAME} iota-bridge
    ```
- Compile from source code
    ...
    ```
    ```sh
    $ cargo build --release --bin iota-bridge
    ```
- Use `curl`/`wget` pre-built binaries (for Linux/AMD64 only).
    ```sh
    $ curl https://iota-releases.s3.us-east-1.amazonaws.com/{SHA}/iota-bridge -o iota-bridge
    ```
- Use pre-built Docker image. Pull from Docker Hub: `iotaledger/iota-tools:{SHA}`


### Run Bridge Node

Running Bridge Node is similar to running a IOTA node using systemd or Ansible. The command to start the Bridge Node is:

```sh
$ RUST_LOG=info,iota_bridge=debug iota-bridge --config-path {BRIDGE-NODE-CONFIG-PATH}
```
...
You can find a full list of Bridge Node metrics and their descriptions in the [`iota-bridge` crate](https://github.com/iotaledger/iota/blob/develop/crates/iota-bridge/src/metrics.rs).
