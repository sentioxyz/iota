...
This guide focuses on using the IOTA CLI `validator` commands. 
...
1. Make sure you have completed all the [prerequisites](../developer/getting-started/iota-install.mdx#prerequisites).

2. Build the `iota` binary, which you need for the genesis ceremony. This step can be done on any machine you like. It does not have to be done on the machine on which you will run the validator.
...
3. Run the following command to set up your IOTA account and CLI environment. 
    ...
    2. If you already set it up, simply make sure 
      ...
      b. `iota.keystore` contains your account key pair.
        ...
        If at this point you can't find where `client.yaml` or `iota.keystore` is or have other questions, read [IOTA Client CLI tutorial](/references/cli/client.mdx).
...
``` bash
$ iota validator display-metadata <validator-address>
```
...
```sh
$ iota validator update-gas-price <gas-price>
```

if the account itself is a validator and holds the Operation Cap. Or 

```sh
$ iota validator update-gas-price --operation-cap-id <operation-cap-id> <gas-price>
```
...
```sh
$ iota validator report-validator <reportee-address>
```
...
```sh
$ iota validator make-validator-info <name> <description> <image-url> <project-url> <host-name> <gas_price>
```
...
``` sh
$ iota validator become-candidate {path-to}validator.info
```
...
```sh
$ iota validator serialize-payload-pop --account-address $ACCOUNT_ADDRESS --protocol-public-key $BLS_PUBKEY
```
```sh
