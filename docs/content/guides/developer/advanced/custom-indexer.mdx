...
You can build custom indexers using the IOTA micro-data ingestion framework. To create an indexer, you subscribe to a checkpoint stream with full checkpoint content. This stream can be one of the publicly available streams from IOTA Foundation, one that you set up in your local environment, or a combination of the two.  
...
The most straightforward stream source is to subscribe to a remote store of checkpoint contents. IOTA Foundation provides the following buckets:
...
- [IOTA internal example](https://github.com/iotaledger/iota/tree/main/crates/iota-data-ingestion/src/): IOTA data ingestion daemon that runs internal pipelines.
- [Production example](https://github.com/iotaledger/iota/tree/main/crates/iotans-indexer/src): IOTA Name Service custom indexer.
- [Using Events](../iota-101/using-events.mdx): Events in IOTA enable you to monitor on-chain activity in near-real time when coupled with a custom indexer.
