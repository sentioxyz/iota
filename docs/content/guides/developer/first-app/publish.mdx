...
Before you can call functions in a Move package (beyond an emulated IOTA execution scenario), that package must be available on the IOTA network. When you publish a package, you are actually creating an immutable IOTA object on the network that anyone can access.

To publish your package to the IOTA network, use the `publish` CLI command in the root of your package. Use the `--gas-budget` flag to set a value for the maximum amount of gas the transaction can cost. If the cost of the transaction is more than the budget you set, the transaction fails and your package doesn't publish. 
...
Your currently active address now has three objects (or more, if you had objects prior to this example). Assuming you are using a new address, running the `iota objects` command reveals what those objects are.
...
After the transaction executes, you can check the status of the `Sword` object by using the `iota client objects` command again. Provided you used your address as the `<TO-ADDRESS>`, you should now see a total of four objects:
...
Congratulations! You have successfully published a package to the IOTA network and modified the blockchain state by using a programmable transaction block. 
...
- [IOTA Client PTB CLI](../../../references/cli/ptb.mdx): The `client ptb` command allows you to specify the transactions for execution in a programmable transaction block directly from your CLI or through bash scripts.
