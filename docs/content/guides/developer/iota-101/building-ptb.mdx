...
This guide explores creating a programmable transaction block (PTB) on IOTA using the TypeScript SDK. For an overview of what a PTB is, see [Programmable Transaction Blocks](/concepts/transactions/prog-txn-blocks.mdx) in the Concepts section. If you don't already have the IOTA TypeScript SDK, follow the [install instructions](https://docs.iota.org/ts-sdk/typescript/install) on the IOTA TypeScript SDK site.

This example starts by constructing a PTB to send IOTA. If you are familiar with the legacy IOTA transaction types, this is similar to a `payIota` transaction. To construct transactions, import the `Transaction` class, and construct it:
...
tx.transferObjects([coin], tx.pure('0xSomeIotaAddress'));
```
...
After you have the Transaction defined, you can directly execute it with a `IotaClient` and `KeyPair` using `client.signAndExecuteTransaction`.
...
To serialize a PTB for sending to a wallet, IOTA recommends using the `tx.serialize()` function, which returns an opaque string representation of the PTB that can be passed from the wallet standard dApp context to your wallet. This can then be converted back into a `Transaction` using `Transaction.from()`.
