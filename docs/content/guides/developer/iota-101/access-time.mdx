...
{@inject: crates/iota-framework/packages/iota-framework/sources/clock.move#fun=timestamp_ms noComments}
...
Expect the `Clock` timestamp to change at the rate the network generates checkpoints, which is about **every 1/4 second** with Mysticeti consensus. Find the current network checkpoint rate on this [public dashboard](https://metrics.iota.io/public-dashboards/4ceb11cc210d4025b122294586961169).
...
{@inject: crates/iota-framework/packages/iota-framework/sources/clock.move#fun=create_for_testing,share_for_testing,increment_for_testing,set_for_testing,destroy_for_testing noComments}

The next example presents a basic test that creates a Clock, increments it, and then checks its value:

{@inject: crates/iota-framework/packages/iota-framework/tests/clock_tests.move#module=iota::clock_tests noComments}
...
{@inject: crates/iota-framework/packages/iota-framework/sources/tx_context.move#fun=epoch_timestamp_ms noComments}
...
{@inject: crates/iota-framework/packages/iota-framework/sources/test/test_scenario.move#fun=later_epoch noComments}
