...
IOTA uses a `IotaKeyPair` to produce the signature, which commits to the Blake2b hash digest of the intent message (`intent || bcs bytes of tx_data`). The signature schemes currently supported are `Ed25519 Pure`, `ECDSA Secp256k1`, `ECDSA Secp256r1`, `Multisig`, and `zkLogin`.

You can instantiate `Ed25519 Pure`, `ECDSA Secp256k1`, and `ECDSA Secp256r1` using `IotaKeyPair` and use it to sign transactions. Note that this guide does not apply to `Multisig` and `zkLogin`, please refer to their own pages ([Multisig](/concepts/cryptography/transaction-auth/multisig) and [zkLogin](https://docs.iota.org/concepts/cryptography/zklogin) respectively) for instructions.
...
There are various ways to instantiate a key pair and to derive its public key and IOTA address using the IOTA TypeScript SDK.
...
There are various ways to instantiate a `IotaKeyPair` and to derive its public key and IOTA address using the IOTA Rust SDK.
