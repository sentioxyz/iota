...
This example walks you through building a coin flip dApp, covering the full end-to-end flow of building your IOTA Move module and connecting it to your React IOTA dApp. This coin flip dApp utilizes verifiable random functions (VRFs) to create a fair coin game on the IOTA blockchain. The user (human) plays against the house (module) and places a bet on either heads or tails. The user then either receives double their bet, or gets nothing, depending on the outcome of the game.
...
- [Move package repository](https://github.com/iotaledger/satoshi-coin-flip)
- [Frontend repository](https://github.com/iota-foundation/satoshi-coin-flip-frontend-example)
...
- **One-time witnesses:** The guide teaches you how to use [one-time witnesses](concepts/iota-move-concepts.mdx#one-time-witness) to ensure only a single instance of the `HouseData` object ever exists.
...
- **Events:** The guide teaches you how to emit events in your contracts, which can be used to track on-chain activity. For more information on events, see [Using Events](../iota-101/using-events.mdx) for practical usage of events on IOTA or [Events in The Move Book](https://move-book.com/programmability/events.html) to learn about event structure and how to emit them in Move.
...
- **MEV attack protection:** The guide introduces you to [MEV attacks](https://github.com/iotaledger/satoshi-coin-flip?tab=readme-ov-file#mev-attack-resistant-single-player-satoshi-smart-contract-flow), how to make your contracts MEV-resistant, and the trade-offs between protection and user experience.
...
1. Seven lines begin with the `use` keyword, which enables this module to use types and functions declared in other modules (in this case, they are all coming from the IOTA standard library).
...
```move title='house_data.move'
  public fun initialize_house_data(house_cap: HouseCap, coin: Coin<IOTA>, public_key: vector<u8>, ctx: &mut TxContext) {
    ...
    let house_data = HouseData {
      ...
      house: ctx.sender(),
      public_key,
      max_stake: 50_000_000_000, // 50 IOTA, 1 IOTA = 10^9.
      min_stake: 1_000_000_000, // 1 IOTA.
      fees: balance::zero(),
...
```move title='house_data.move'
  public fun top_up(house_data: &mut HouseData, coin: Coin<IOTA>, _: &mut TxContext) {
...
- `top_up`: Add to the balance of the house to ensure that there is enough IOTA for future games.
...
```move title='house_data.move'
  ...
  public(package) fun borrow_balance_mut(house_data: &mut HouseData): &mut Balance<IOTA> {
    &mut house_data.balance
  }

  public(package) fun borrow_fees_mut(house_data: &mut HouseData): &mut Balance<IOTA> {
    &mut house_data.fees
  }
...
module satoshi_flip::single_player_satoshi {
  ...
  use iota::iota::IOTA;
  use iota::bls12381::bls12381_min_pk_verify;
  use iota::event::emit;
...
```move title='single_player_satoshi.move'
  ...
  fun internal_start_game(guess: String, counter: &mut Counter, coin: Coin<IOTA>, house_data: &mut HouseData, fee_bp: u16, ctx: &mut TxContext): (ID, Game) {
...
You can read more about both versions of the game, and view the full source code for all the modules in the [Satoshi Coin Flip repository](https://github.com/iotaledger/satoshi-coin-flip).
...
Next, configure the IOTA CLI to use `testnet` as the active environment, as well. If you haven't already set up a `testnet` environment, do so by running the following command in a terminal or console:
...
To skip building the frontend and test out your newly deployed package, use the provided [Satoshi Coin Flip Frontend Example repository](https://github.com/iota-foundation/satoshi-coin-flip-frontend-example) and follow the instructions in the example's `README.md` file
...
- Tooling: [IOTA TypeScript SDK](https://docs.iota.org/ts-sdk/typescript) for basic usage on how to interact with IOTA using TypeScript.
...
- Tooling: [`@iota/dapp`](https://docs.iota.org/ts-sdk/dapp-kit/create-dapp), used within this project to quickly scaffold a React-based IOTA dApp.
...
The following frontend code snippets include only the most relevant sections. Refer to the [Satoshi Coin Flip Frontend Example repository](https://github.com/iota-foundation/satoshi-coin-flip-frontend-example) for complete source code.
...
<form
  onSubmit={(e) => {
    ...
    const [houseStakeCoin] = txb.splitCoins(txb.gas, [
      NANOS_PER_IOTA * BigInt(houseStake),
    ]);
    ...
    txb.moveCall({
      ...
      arguments: [
        ...
        // https://docs.iota.org/ts-sdk/typescript/transaction-building/basics#pure-values
        txb.pure(
          bcs
            .vector(bcs.U8)
...
To use a [programmable transaction block](/concepts/transactions/prog-txn-blocks.mdx) (PTB) in Iota, create a `Transaction`. To initiate a Move call, you must know the global identifier of a public function in your smart contract. The global identifier usually takes the following form:
...
// This hook is to demonstrate how to use `@iota/dapp-kit` React hook to query data
// besides using IotaClient directly
export function useFetchCounterNft() {
  ...
  const { data, isLoading, isError, error, refetch } = useIotaClientQuery(
    'getOwnedObjects',
    {
...
// Player stake
const [stakeCoin] = txb.splitCoins(txb.gas, [NANOS_PER_IOTA * BigInt(stake)]);
...
Event subscriptions are deprecated. To learn future-safe methods to work with events, see [Using Events](../iota-101/using-events.mdx).
...
export function HouseFinishGame() {
  ...
  useEffect(() => {
    ...
    const unsub = iotaClient.subscribeEvent({
      filter: {
  ...
  }, [housePrivHex, houseDataId, iotaClient]);

  return null;
}
```

To get the underlying `IotaClient` instance from the SDK, use `useIotaClient()`. You want to subscribe to events whenever the `HouseFinishGame` component loads. To do this, use the React hook `useEffect()` from the core React library.

`IotaClient` exposes a method called `subscribeEvent()` that enables you to subscribe to a variety of event types. `IotaClient::subscribeEvent()` is actually a thin wrapper around the RPC method [`iotax_subscribeEvent`](/iota-api-ref#iotax_subscribeevent).
...
Congratulations, you completed the frontend. You can carry the lessons learned here forward when using the dApp Kit to build your next IOTA project.
