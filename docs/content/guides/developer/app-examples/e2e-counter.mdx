...
This example walks you through building a basic distributed counter app, covering the full end-to-end flow of building your IOTA Move module and connecting it to your React IOTA dApp. The app allows users to create counters that anyone can increment, but only the owner can reset. 
...
[Example source code](https://github.com/iotaledger/iota/blob/60bb8bdc274b9e5706fd916cd84c13f81e832529/sdk/create-dapp/templates/react-e2e-counter)
...
{@inject: github:iotaledger/ts-sdks/packages/create-dapp/templates/react-e2e-counter/move/counter/Move.toml}
...
{@inject: github:iotaledger/ts-sdks/packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move#struct=Counter noComments}

- The `Counter` type stores the address of its `owner`, its current `value`, and its own `id`.

### Creating `Counter`

{@inject: github:iotaledger/ts-sdks/packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move#fun=create noComments}
...
{@inject: github:iotaledger/ts-sdks/packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move#fun=increment noComments}

The `increment` function accepts a mutable reference to any shared `Counter` object and increments its `value` field.

{@inject: github:iotaledger/ts-sdks/packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move#fun=set_value noComments}
...
Learn more about taking [object references as input](../../../concepts/iota-move-concepts.mdx#entry-points)
...
{@inject: github:iotaledger/ts-sdks/packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move noComments}

:::checkpoint

Your smart contract is complete. You should be able to run the `iota move build` command from `react-e2e-counter/move/counter` and receive a response similar to the following:
```sh
...
BUILDING counter
```
You always run `iota move build` at the same level as your `Move.toml` file. After a successful build, you now have a `build` folder inside `react-e2e-counter/move/counter`.
...
<Tabs groupId="packagemanager">
  <TabItem label="PNPM" value="pnpm">
    ```sh
    $ pnpm create @iota/dapp --template react-e2e-counter
    ```
  </TabItem>
  <TabItem label="Yarn" value="yarn">
    ```sh
    $ yarn create @iota/dapp --template react-e2e-counter
    ```
  </TabItem>
</Tabs>
...
<details>
  ...
  {@inject: github:iotaledger/ts-sdks/packages/create-dapp/templates/react-e2e-counter/src/networkConfig.ts noComments}
</details>
...
<details>
  ...
  {@inject: github:iotaledger/ts-sdks/packages/create-dapp/templates/react-e2e-counter/src/CreateCounter.tsx noComments}
</details>

The `create` function now creates a new IOTA `Transaction` and calls the `create` function from your Move module. The PTB is then signed and executed via the `useSignAndExecuteTransaction` hook. The `onCreated` callback is called with the new counter's ID when the transaction is successful.
...
<details>
  ...
  {@inject: github:iotaledger/ts-sdks/packages/create-dapp/templates/react-e2e-counter/src/App.tsx noComments}
</details>
