...
The `@iota/iota-sdk/transaction` SDK exports the `SerialTransactionExecutor` class, which you can use
...
Construct the transaction separately with the `@iota/iota-sdk` library and then sign it with the private key of the user. Use the `iota:signTransaction` feature to achieve this:

```tsx
wallet.features[iota:signTransaction].signTransaction(<Transaction>, <WalletAccount>);
```
...
To execute the transaction, use `IotaClient` from `@iota/iota-sdk`:
...
IOTA Foundation offers a bare bones scaffold for React-based applications called `@iota/dapp-kit`. See the [dApp Kit documentation](https://docs.iota.org/ts-sdk/dapp-kit) for more informqtion.
