...
The following example uses the [IOTA TypeScript SDK](https://docs.iota.org/ts-sdk/typescript) to connect via websocket to listen to events.

```javascript
const local_client = new IotaClient({ url: 'https://fullnode.mainnet.iota.io:443/' });
...
deepbook_client.iotaClient.subscribeEvent({
