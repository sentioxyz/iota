    ...
    match args.command {
        ...
        BridgeCommand::Governance {
            ...
            let iota_bridge_client =
                IotaClient::<IotaSdkClient>::new(&config.iota_rpc_url, metrics.clone()).await?;
        ...
        BridgeCommand::ViewIotaBridge {
            ...
            for (_, member) in move_type_bridge_committee.members {
                ...
                let name = names.get(&iota_address).cloned().unwrap_or(url.clone());
        ...
        BridgeCommand::Client { config_path, cmd } => {
            ...
            let metrics = Arc::new(BridgeMetrics::new_for_testing());
            let iota_bridge_client =
                IotaClient::<IotaSdkClient>::new(&config.iota_rpc_url, metrics).await?;
