...
impl CallTraceWithSource {
    pub fn from(call_trace: InternalCallTrace, trace_v2: bool) -> Self {
        ...
        CallTraceWithSource {
            ...
            contract_name: module_name.unwrap_or(IOTA_FRAMEWORK_ADDRESS.to_string().as_str()).to_string(),
            to: to_account.unwrap().to_string(),
            function_name: format!(
                "{}::{}",
                to_module_name.unwrap_or(IOTA_FRAMEWORK_ADDRESS.to_string().as_str()).to_string(),
