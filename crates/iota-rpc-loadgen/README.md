...
To try this locally, refer to the [docs](https://docs.iota.org/guides/developer/getting-started/local-network). Recommend setting `database-url` to an env variable. Note: run `RUST_LOG="consensus=off" cargo run iota -- start --with-faucet --force-regenesis --with-indexer` to rebuild.
