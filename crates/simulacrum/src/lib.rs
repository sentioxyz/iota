...
use iota_types::base_types::{AuthorityName, ObjectID, ObjectRef, VersionNumber};
use iota_types::crypto::{get_account_key_pair, AccountKeyPair, AuthoritySignature};
...
impl<R, S: store::SimulatorStore> Simulacrum<R, S> {
    ...
    /// let (account, kp, gas) = simulacrum.funded_account(NANOS_PER_IOTA).unwrap();
    ///
    /// // `account` is a fresh IotaAddress that owns a Coin<IOTA> object with single IOTA in it,
    /// // referred to by `gas`.
    /// // ...
    /// # }
    /// ```
    pub fn funded_account(
        &mut self,
        amount: u64,
    ) -> Result<(IotaAddress, AccountKeyPair, ObjectRef)> {
