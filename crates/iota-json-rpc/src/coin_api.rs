            ...
            let cursor = match cursor {
                Some(c) => {
                    let decoded = CoinCursor::decode(&c).ok_or_else(|| {
                        IotaRpcInputError::GenericInvalid("invalid cursor".to_string())
                    })?;

                    if coin_type_tag.to_string() != decoded.coin_type {
                        return Err(
                            IotaRpcInputError::GenericInvalid("invalid cursor".to_string()).into(),
                        );
                    }
                    (
                        decoded.coin_type,
...
mod tests {
    ...
    mock! {
        ...
        impl TransactionKeyValueStoreTrait for KeyValueStore {
            ...
            async fn multi_get_events_by_tx_digests(&self,digests: &[TransactionDigest]) -> IotaResult<Vec<Option<TransactionEvents>>>;
        }
    }
