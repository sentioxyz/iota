...
impl ReadApiServer for ReadApi {
    ...
    async fn verify_zklogin_signature(
        ...
        let zklogin_env_native = match self
            ...
            .chain()
        {
            iota_protocol_config::Chain::Mainnet | iota_protocol_config::Chain::Testnet => {
                ZkLoginEnv::Prod
            }
            _ => ZkLoginEnv::Test,
        };
        let GenericSignature::ZkLoginAuthenticator(zklogin_sig) =
            ...
            return Err(IotaRpcInputError::GenericNotFound(
...
fn convert_to_response(
    ...
    if let Some(transaction) = cache.transaction {
        ...
        if opts.show_input {
            response.transaction = Some(IotaTransactionBlock::try_from(
