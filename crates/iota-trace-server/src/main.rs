...
use iota_replay::call_trace::CallTraceWithSource;
...
struct NetworkConfig {
    local_exec: iota_replay::replay::LocalExec,
}

impl NetworkConfig {
    async fn new(rpc_url: String) -> Self {
        Self {
            local_exec: iota_replay::init_for_tracer(rpc_url).await.unwrap(),
        }
    }
}
...
async fn main() {
    ...
    let rpc_urls = env::args().nth(1).unwrap_or_else(|| "iota_mainnet=https://api.mainnet.iota.cafe:443".to_string());
...
async fn call_trace(
    ...
    let trace_result = iota_replay::execute_call_trace(
