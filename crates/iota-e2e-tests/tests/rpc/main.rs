...
async fn transfer_coin(
    context: &iota_sdk::wallet_context::WalletContext,
) -> iota_sdk_types::TransactionDigest {
...
async fn stake_with_validator(
    ...
    let validator_address = cluster.swarm.config().validator_configs()[0].iota_address();
    let txn = context.sign_transaction(
        &iota_test_transaction_builder::TestTransactionBuilder::new(sender, gas_object, gas_price)
