...
fn proto_to_response(
    proto: iota_rpc_api::proto::node::v2alpha::ResolveTransactionResponse,
...
async fn resolve_transaction_simple_transfer() {
    ...
    let mut alpha_client =
        iota_rpc_api::proto::node::v2alpha::node_service_client::NodeServiceClient::connect(
...
async fn resolve_transaction_mutable_shared_object() {
    ...
    let validator_address = test_cluster.swarm.config().validator_configs()[0].iota_address();
